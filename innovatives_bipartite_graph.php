...
foreach ($scholars as $scholar) {
    if (count($scholarsMatrix[$scholar[$nodes1_id]]['cooc'])>0){        
    foreach ($scholar['keywords_ids'] as $keywords)     
    if ($keywords!=null){    
    $edgeid+=1;
    $gexf.='<edge id="' . $edgeid . '"' . ' source="D::' . $scholar[$nodes1_id] . '" ' .
            ' target="N::' . trim($keywords) . '" weight="1">' . "\n";
    ...
    foreach ($neighbors as $neigh_id => $occ) {        
        if ($neigh_id!=$nodeId1) {
            ...
            $gexf.='<edge id="'.$edgeid.'"'.' source="N::'.$nodeId1.'" '.
                    ' target="N::'.trim($neigh_id)  .'" weight="'.($occ/$term['occurrences']).'">'."\n";
